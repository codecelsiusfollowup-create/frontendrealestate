...
const VisitedPage = () => {
  ...
  const fetchVisited = async () => {
    ...
    try {
      ...
      const { data } = await axios.get(`${BACKEND}/api/leads/visits${qs}`);
  ...
  const handleStatusChange = async (leadId, idx, newStatus) => {
    try {
      setUpdating(true);
      await axios.patch(`${BACKEND}/api/leads/update-visit-status/${leadId}/${idx}`, {
        status: newStatus,
      });
      await fetchVisited();
    } catch (e) {
  ...
  const TableRow = ({ lead }) => (
    <tr className="hover:bg-slate-50 transition-colors border-b border-slate-100">
        ...
        <div className="space-y-2">
          {lead.visits.map((v, i) => (
            <div key={i} className="text-xs text-slate-500">
              ...
              {v.location && <span className="ml-2 text-slate-600">📍 {v.location}</span>}
            </div>
          ))}
        </div>
      </td>
      ...
      {/* Remark column */}
      <td className="px-5 py-4 text-xs text-slate-600 italic">
        {lead.visits.at(-1)?.notes || <span className="text-slate-400">—</span>}
      </td>

      <td className="px-5 py-4">
        <Badge status={lead.visits.at(-1)?.status || "pending"} />
      </td>
      ...
      <DashboardLayout>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8 py-10">
          <div className="rounded-xl border border-dashed border-slate-300 bg-slate-50 p-12 text-center">
            <p className="text-slate-600">No visited leads found.</p>
            <button
    ...
    <DashboardLayout>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8 py-10">
        <div className="md:flex md:items-center md:justify-between mb-8">
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-slate-900">Visited Leads</h1>
            <p className="mt-1 text-sm text-slate-500">Track and update every site-visit interaction.</p>
          </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
          <table className="min-w-full divide-y divide-slate-200">
            <thead className="bg-slate-100">
              <tr>
                {["Lead Name", "Email", "Phone", "Visits", "Remark", "Latest Status", "Action"].map(
                  (h) => (
                    <th
                      key={h}
                      scope="col"
